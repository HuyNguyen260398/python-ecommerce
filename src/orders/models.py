...
class OrderQuerySet(models.query.QuerySet):
    ...
    def get_sales_breakdown(self):
        ...
        recent_order_data = recent.order_totals_data()
        recent_cart_data = recent.cart_totals_data()
        shipped = recent.by_status(status='shipped')
        shipped_data = shipped.order_totals_data()
        paid = recent.by_status(status='paid')
        paid_data = paid.order_totals_data()
    ...
    def by_date(self):
        now = timezone.now()
        # now = timezone.now() - datetime.timedelta(days=7) # filter in a week/last 7 days
        # return self.filter(updated__day__gte=now.day) # gte = greater than or equal
        return self.filter(updated__day=now.day)

    def order_totals_data(self):
        return self.aggregate(
            Sum("total"),
            Avg("total"),
        )

    def cart_totals_data(self):
        return self.aggregate(
...
class Order(models.Model):
    ...
    shipping_total = models.DecimalField(default=5.00,
                                         max_digits=100,
                                         decimal_places=2)
