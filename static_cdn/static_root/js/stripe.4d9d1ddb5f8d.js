$(document).ready(function(){
  ...
  var stripeFormModuleToken = stripeFormModule.attr('data-token')
  var stripeFormModuleNextUrl = stripeFormModule.attr('data-next-url')
  var stripeFormModuleBtnTitle = stripeFormModule.attr('data-btn-title') || "Add Card"
  var stripeTemplate = $.templates('#stripeTemplate')
  var stripeTemplateDataContext = {
    publishKey: stripeFormModuleToken,
    nextUrl: stripeFormModuleNextUrl,
    btnTitle: stripeFormModuleBtnTitle
  }
  ...
  if (paymentForm.length > 1){
    ...
    //   // Display new btn UI
    //   var loadTime = 1500
    //   var errorHtml = "<i class='fas fa-exclamation-triangle'></i>"
    //   var errorClasses = "btn btn-danger disabled my-3"
    //   var loadingHtml = "<i class='fas fa-circle-notch fa-spin'></i> Loading..."
    ...
    var btnLoadDefaultClasses = btnLoad.attr('class')
      ...
      var loadTime = 1500
      var currentTimeout;
      var errorHtml = "<i class='fas fa-exclamation-triangle'></i> An error occurred!"
      var errorClasses = "btn btn-danger disabled my-3"
      var loadingHtml = "<i class='fas fa-circle-notch fa-spin'></i> Loading..."
      var loadingClasses = "btn btn-success disabled my-3"
    ...
    function stripeTokenHandler(nextUrl, token){
      ...
      $.ajax({
        ...
        error: function(error){
          $.alert({title: 'An error has occurred!', content: "Please try adding your card again!"})
          btnLoad.html(btnLoadDefaultHtml)
          btnLoad.attr('class', btnLoadDefaultClasses)
        }
      })
